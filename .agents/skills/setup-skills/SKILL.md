...
> Explainer: When the `triage` skill processes an incoming issue, it moves it through a state machine — needs evaluation, waiting on reporter, ready for an AFK agent (general or TDD), ready for a human, or won't fix. To do that, it needs to apply labels (or the equivalent in your issue tracker) that match strings *you've actually configured*. If your repo already uses different label names (e.g. `bug:triage` instead of `needs-triage`), map them here so the skill applies the right ones instead of creating duplicates.
...
- `ready-for-tdd-agent` — fully specified with behavioral acceptance criteria, AFK-ready for a TDD-discipline agent (red→green)
- `ready-for-human` — needs human implementation
- `wontfix` — will not be actioned

`ready-for-agent` and `ready-for-tdd-agent` are mutually exclusive: the former is dispatched by `parallel-issues`, the latter by `parallel-tdd`.
